...
def play(strategy0, strategy1, goal=GOAL_SCORE):
    ...
    while score<100 and opponent_score<100:
        if who==0:
            ...
            if can_swap(score,opponent_score) == True:
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    for i in range (1,11):
        answer = make_averaged(roll_dice)(1,dice)
        print(i,"dice scores",answer * i,"on average")
...
def swap_strategy(score, opponent_score):
    ...
    if score + take_turn(0,opponent_score) == opponent_score // 2:
        return 0
    elif score + take_turn(0,opponent_score) == opponent_score * 2:
        return BASELINE_NUM_ROLLS
...
def final_strategy(score, opponent_score):
    ...
    """
    Make Swap       ) Tries to get a beneficial swap.
    Make 4sided_dice) Tries to get the opponent a four_sided_dice.
    if losing roll higher 
    """
    # BEGIN Question 10
    ...
    #Make swap :
    if can_swap(score + 1, opponent_score) and ((score + 1) < opponent_score):
        return 10

    if can_swap(score + free_bacon, opponent_score) and ((score + free_bacon) < opponent_score):
        return 0

    #Make 4_sided_dice (Hogwild):
    ...
    if score - opponent_score > 33:
        if dice == four_sided:
            ...
            return swap_strategy(score, opponent_score)
    # if in the lead roll higher then normal
    if opponent_score - score > 16:
        if dice == four_sided:
            return swap_strategy(score, opponent_score)
        else:
            return 8
    # play it safe towards the end 
    if score > 80:
        return swap_strategy(score, opponent_score)
